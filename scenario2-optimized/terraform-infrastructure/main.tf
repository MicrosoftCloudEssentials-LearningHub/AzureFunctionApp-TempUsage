# main.tf
# Scenario 2: Optimized Mounted Deployment with Minimal Disk Write Access
# Test best practices for efficient disk usage and performance
...
# Application Insights - With sampling enabled for optimized scenario
resource "azurerm_application_insights" "appinsights" {
  name                = "${var.base_app_insights_name}${var.resource_suffix}"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name
  application_type    = "web"
  sampling_percentage = 5 # Enable sampling - optimized for low disk usage
...
resource "random_password" "sql_admin_password" {
  length           = var.sql_password_length
  special          = true
  override_special = "!#$%&*()-_=+[]{}<>:?"
  min_lower        = 1
  min_upper        = 1
...
# Windows Function App - Optimized for mounted package deployment
resource "azurerm_windows_function_app" "function_app" {
  ...
  site_config {
    ...
    # Health check path with eviction time
  ...
  # App settings for optimized scenario
  app_settings = {
    ...
    # Disable dashboard logging to storage
    ...
    "ApplicationInsights:SamplingTelemetry"   = "true"
    "ApplicationInsights:SamplingPercentage"  = "5"
  ...
  # Allow WEBSITE_RUN_FROM_PACKAGE to be updated after creation
  lifecycle {
    ignore_changes = []
  }
}
